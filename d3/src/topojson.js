...
(function (global, factory) {
  ...
  var transform = function (transform) {
    ...
    var x0, y0, kx = transform.scale[0], ky = transform.scale[1], dx = transform.translate[0],
  ...
  var stitch = function (topology, arcs) {
    ...
    function ends(i) {
      ...
      if (topology.transform) p1 = [0, 0], arc.forEach(function (dp) { p1[0] += dp[0], p1[1] += dp[1]; }); else p1 = arc[arc.length - 1];
  ...
  function extractArcs(topology, object$$1, filter) {
    ...
    geomsByArc.forEach(filter == null ? function (geoms) { arcs.push(geoms[0].i); } : function (geoms) { if (filter(geoms[0].g, geoms[geoms.length - 1].g)) arcs.push(geoms[0].i); });
  ...
  function mergeArcs(topology, objects) {
    ...
    return {
      type: 'MultiPolygon', arcs: groups.map(function (polygons) {
        var arcs = [], n;
  ...
  var neighbors = function (objects) {
    ...
    function geometry(o, i) {
      if (o.type === 'GeometryCollection') o.geometries.forEach(function (o) { geometry(o, i); }); else if (o.type in geometryType) geometryType[o.type](o.arcs, i);
    }
  ...
  var untransform = function (transform) {
    ...
    return function (input, i) {
      if (!i) x0 = y0 = 0;
      var j = 2, n = input.length, output = new Array(n), x1 = Math.round((input[0] - dx) / kx),
  ...
  var quantize = function (topology, transform) {
    ...
    if (!transform || !transform.scale) {
      ...
      transform = {
        scale: [x1 - x0 ? (x1 - x0) / (n - 1) : 1, y1 - y0 ? (y1 - y0) / (n - 1) : 1], translate: [x0, y0]
      };
    }
    else {
      box = topology.bbox;
    ...
    return {
      type: 'Topology', bbox: box, transform: transform, objects: outputs, arcs: topology.arcs.map(quantizeArc)
    };
  };
...
// Computes the bounding box of the specified hash of GeoJSON objects.
  ...
  var hashmap = function (size, hash, equal, keyType, keyEmpty, valueType) {
    ...
    var keystore = new keyType(size = 1 << Math.max(4, Math.ceil(Math.log(size) / Math.LN2))),
      valstore = new valueType(size), mask = size - 1;
    ...
    return {
      set: set, maybeSet: maybeSet, // set if unset
      get: get, keys: keys
    };
  };
...
// line is never rotated.
  var join = function (topology) {
    var coordinates = topology.coordinates, lines = topology.lines, rings = topology.rings, indexes = index(),
      visitedByIndex = new Int32Array(coordinates.length), leftByIndex = new Int32Array(coordinates.length),
      rightByIndex = new Int32Array(coordinates.length), junctionByIndex = new Int8Array(coordinates.length),
      junctionCount = 0, // upper bound on number of junctions
      i, n, previousIndex, currentIndex, nextIndex;
...
// to remove exact duplicate arcs.
  var cut = function (topology) {
    var junctions = join(topology), coordinates = topology.coordinates, lines = topology.lines, rings = topology.rings,
      next, i, n;
    ...
    for (i = 0, n = rings.length; i < n; ++i) {
      var ring = rings[i], ringStart = ring[0], ringMid = ringStart, ringEnd = ring[1],
...
// Given a cut topology, combines duplicate arcs.
  var dedup = function (topology) {
    var coordinates = topology.coordinates, lines = topology.lines, line, rings = topology.rings, ring,
      arcCount = lines.length + rings.length, i, n;
    ...
    var arcsByEnd = hashmap(arcCount * 2 * 1.4, hashPoint, equalPoint), arcs = topology.arcs = [];
...
// lines or rings.
  var extract = function (objects) {
    ...
    return {
      type: 'Topology', coordinates: coordinates, lines: lines, rings: rings, objects: objects
    };
  };
...
// Each output geometry object is a shallow copy of the input (e.g., properties, coordinates)!
  ...
  var prequantize = function (objects, bbox, n) {
    var x0 = bbox[0], y0 = bbox[1], x1 = bbox[2], y1 = bbox[3], kx = x1 - x0 ? (n - 1) / (x1 - x0) : 1,
    ...
    function quantizePoints(input, m) {
      var i = -1, j = 0, n = input.length, output = new Array(n), // pessimistic
        pi, px, py, x, y;
    ...
    return {
      scale: [1 / kx, 1 / ky], translate: [x0, y0]
    };
  };
...
// meaning FeatureCollection, a Feature or a geometry object.
  var topology = function (objects, quantization) {
    var bbox = bounds(objects = geometry(objects)),
      ...
      topology = dedup(cut(extract(objects))), coordinates = topology.coordinates,
  ...
  var prune = function (topology) {
    var oldObjects = topology.objects, newObjects = {}, oldArcs = topology.arcs, oldArcsLength = oldArcs.length,
      oldIndex = -1, newIndexByOldIndex = new Array(oldArcsLength), newArcsLength = 0, newArcs, newIndex = -1, key;
    ...
    return {
      type: 'Topology', bbox: topology.bbox, transform: topology.transform, objects: newObjects, arcs: newArcs
    };
  };
  ...
  var filter = function (topology, filter) {
    ...
    return prune({
      type: 'Topology', bbox: topology.bbox, transform: topology.transform, objects: newObjects, arcs: topology.arcs
    });
  };
  ...
  var presimplify = function (topology, weight) {
    var point = topology.transform ? transform(topology.transform) : copy, heap = newHeap();
    ...
    return {
      type: 'Topology', bbox: topology.bbox, objects: topology.objects, arcs: arcs
    };
  };
  ...
  var simplify = function (topology, minWeight) {
    ...
    return {
      type: 'Topology', transform: topology.transform, bbox: topology.bbox, objects: topology.objects, arcs: arcs
    };
  };
  ...
  function halfArea(ring, closed) {
    var i = 0, n = ring.length, sum = 0, point = ring[closed ? i++ : n - 1], lambda0, lambda1 = point[0] * radians,
      phi1 = (point[1] * radians) / 2 + quarterPi, cosPhi0, cosPhi1 = cos(phi1), sinPhi0, sinPhi1 = sin(phi1);
    ...
    for (; i < n; ++i) {
      ...
      var dLambda = lambda1 - lambda0, sdLambda = dLambda >= 0 ? 1 : -1, adLambda = sdLambda * dLambda,
        k = sinPhi0 * sinPhi1, u = cosPhi0 * cosPhi1 + k * cos(adLambda), v = k * sdLambda * sin(adLambda);
      sum += atan2(v, u);
    }
