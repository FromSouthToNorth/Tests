...
function utilKeybinding(namespace) {
  ...
  function testBindings(d3_event, isCapturing) {
    let didMatch = false;
    let bindings = Object.keys(_keybindings)
      .map(function(id) {
  ...
  function bubble(d3_event) {
    let tagName = d3.select(d3_event.target)
      .node().tagName;
  ...
  keybinding.off = function(codes, capture) {
    let arr = utilArrayUniq([].concat(codes));

    for (let i = 0; i < arr.length; i++) {
      let id = arr[i] + (capture ? '-capture' : '-bubble');
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      for (let j = 0; j < matches.length; j++) {
        ...
        if (matches[j] in utilKeybinding.modifierCodes) {
          let prop = utilKeybinding.modifierProperties[utilKeybinding.modifierCodes[matches[j]]];
